...
export const onRequest = async (context: EventContext<Env>): Promise<Response> => {
  ...
  try {
    ...
    if (context.request.method === 'POST') {
      ...
      const reportData = await context.request.json() as Omit<CreateUserReportInput, 'reporter_user_id'>
      
      // Validate required fields
      if (!reportData.reported_item_type || !reportData.reported_item_id || !reportData.report_reason) {
        return createErrorResponse('Missing required fields: reported_item_type, reported_item_id, report_reason', 400)
      }
      
      // Validate reported_item_type
      if (!['petition', 'signature'].includes(reportData.reported_item_type)) {
        return createErrorResponse('Invalid reported_item_type. Must be "petition" or "signature"', 400)
      }
      
      // Validate report_reason
      const validReasons = ['spam', 'inappropriate_content', 'harassment', 'misinformation', 'hate_speech', 'violence', 'copyright_violation', 'other']
      if (!validReasons.includes(reportData.report_reason)) {
        return createErrorResponse(`Invalid report_reason. Must be one of: ${validReasons.join(', ')}`, 400)
      }
