  ...
  try {
    ...
    if (context.request.method === 'PUT') {
      ...
      if (contentType.includes('multipart/form-data')) {
        ...
        if (formData.get('description')) petitionData.description = formData.get('description') as string
        ...
        if (formData.get('target_count')) petitionData.target_count = parseInt(formData.get('target_count') as string)
        if (formData.get('category_ids')) petitionData.category_ids = JSON.parse(formData.get('category_ids') as string || '[]')
        if (formData.get('status')) petitionData.status = formData.get('status') as 'active' | 'completed' | 'closed'
